...
struct segtree{
    ...
    int dfs(int ans) {
        ...
        if (rb - lb != 1) {
            ...
            //cout <<lb << " " << rb <<  "PIHNULOVOTVET" << ans << endl;
            return ans;
        }
        //cout << lb << " " << rb << " after_dfs " << ans << " ";
    ...
    for (auto x : mmop) {
        for (int i = 0; i < x.second.size(); i++) {
            if (i < mmcl[x.first].size()) {
                ...
                //cout << x.first.first + 1 << " " << x.first.second + 1 << " : " << x.second[i] << " " << mmcl[x.first][i] + 1 << endl;
            }
            else{
